...
let state = 0;
let selector, secretWord, reveal, m, hint, cate;
...
function game() {
  ...
  if (secretWord.length < 13) {
    ...
    function key(e) {
      ...
      e.target.style.backgroundColor = "#e4bc00";
      e.target.style.border = "#e4bc00";
...
nextButton.addEventListener("click", function () {
  ...
  if (category.indexOf(cate) > -1) {
    category.splice(category.indexOf(cate), 1);
  }
  ...
  reveal = "";
  m = 0;
  const keyClear1 = document.getElementsByClassName("keyClear1");
  const keyClear2 = document.getElementsByClassName("keyClear2");
  console.log(keyClear1);
  for (let i = 0; i < keyClear1.length; i++) {
    keyClear1[i].removeAttribute("style");
  }
  for (let i = 0; i < keyClear2.length; i++) {
    keyClear2[i].removeAttribute("style");
  }
